...
If the {vmhost} is registered as a Salt minion, a final configuration step is needed to gather all the guest VMs defined on the {vmhost}:
...
To display a {vmguest} VNC or Spice graphical console, the virtual host corresponding port needs to be reachable by the server.
